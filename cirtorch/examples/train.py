...
training_dataset_names = ['retrieval-SfM-120k','cdvs_train_retrieval']
test_datasets_names = ['oxford5k', 'paris6k', 'roxford5k', 'rparis6k', 'cdvs_test_retrieval']
test_whiten_names = ['retrieval-SfM-30k', 'retrieval-SfM-120k']

model_names = sorted(name for name in models.__dict__
                     if name.islower() and not name.startswith("__")
                     and callable(models.__dict__[name]))
pool_names = ['mac', 'spoc', 'gem', 'hnip']
...
parser.add_argument('--sample-diy', action='store_true',
                    help='using diy sample method')

parser.add_argument('--using-cdvs', default=0.0, type=float,
                    help='using cdvs global vector while pick negtive and if -1, using loss to parameter global')

parser.add_argument('--test-datasets', '-td', metavar='DATASETS', default='roxford5k',
...
parser.add_argument('--query-size', '-qs', default=17645, type=int, metavar='N',
                    help='number of queries randomly drawn per one train epoch (default: 2000)')
parser.add_argument('--pool-size', '-ps', default=2000, type=int, metavar='N',
...
def main():
    ...
    directory += "_{}_lr{:.1e}_wd{:.1e}_gid{}/".format(args.optimizer, args.lr, args.weight_decay,args.gpu_id)
    directory += "_nnum{}_qsize{}_psize{}/".format(args.neg_num, args.query_size, args.pool_size)
    directory += "_samplediy{}_usingcdvs{}/".format(args.sample_diy, args.using_cdvs)
    ...
    train_dataset = TuplesDataset(
        ...
        transform=transform,
        sample_diy=args.sample_diy,
        using_cdvs=args.using_cdvs
    )
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if epoch==start_epoch:
            temp_loss=1
        loss = train(train_loader, model, criterion, optimizer, epoch, temp_loss)
        temp_loss=loss
...
def train(train_loader, model, criterion, optimizer, epoch, temp_loss):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses = AverageMeter()

    # create tuples for training
    avg_neg_distance = train_loader.dataset.create_epoch_tuples(model, temp_loss)
...
def test(datasets, net):
    ...
    if args.test_whiten:
        ...
        wvecs = extract_vectors(net, images, image_size, transform,print_freq=10,batchsize=20)  # implemented with torch.no_grad
    ...
    for dataset in datasets:
        ...
        if len(ms) > 1 and net.meta['pooling'] == 'gem' and not net.meta['regional'] and not net.meta['whitening']:
            ...
            msp = 1
        vecs = extract_vectors(net, images, image_size, transform, bbxs, ms=ms, msp=msp, print_freq=1000,batchsize=1)  # implemented with torch.no_grad
        print('>> {}: query images...'.format(dataset))
        qvecs = extract_vectors(net, qimages, image_size, transform, bbxs, ms=ms, msp=msp,  print_freq=1000,batchsize=1)  # implemented with torch.no_grad
