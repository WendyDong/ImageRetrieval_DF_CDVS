...
from cirtorch.layers.pooling import MAC, SPoC, GeM, RMAC, Rpool, HNIP
...
FEATURES_NOTOWN = [
    'resnet50','se_resnext101_32x4d','se_resnext50_32x4d'
]
...
class ImageRetrievalNet(nn.Module):
    ...
    def __init__(self, features, lwhiten, pool, whiten, meta, poolname):
    ...
    def forward(self, x):
        ...
        if self.poolname=='hnip':
            x90 = x.transpose(2, 3).flip(2)
            x180 = x.flip(2).flip(3)
            x270 = x.transpose(2, 3).flip(3)

            o90 = self.features(x90)
            o180 = self.features(x180)
            o270 = self.features(x270)
            # o = torch.stack((o, o90, o180, o270))
...
def init_network(params):
    ...
    if pretrained:
        if architecture not in FEATURES:
            ...
            if architecture not in FEATURES_NOTOWN:
    ...
    if architecture in FEATURES_NOTOWN:
        own_model = pretrainedmodels.__dict__[architecture](num_classes=1000, pretrained='imagenet')
        features = list(own_model.children())[:-2]
    ...
    net = ImageRetrievalNet(features, lwhiten, pool, whiten, meta,poolname=pooling)
    ...
    if pretrained and architecture in FEATURES:
        ...
        if architecture in FEATURES_NOTOWN:
            net.features.load_state_dict(pretrainedmodels.__dict__[architecture](num_classes=1000, pretrained='imagenet'))
        else:
            net.features.load_state_dict(model_zoo.load_url(FEATURES[architecture], model_dir=model_dir))

    return net

def extract_vectors(net, images, image_size, transform, bbxs=None, ms=[1], msp=1, print_freq=10, batchsize=300):
    ...
    with torch.no_grad():
        ...
        for i, input in enumerate(loader):
            ...
            if len(ms) == 1:
                if batchsize!=1:
                    if (i+1) != len(loader):
                        vecs[:, i*batchsize:(i+1)*batchsize] = extract_ss(net, input)
                    else:
                        vecs[:, i * batchsize:len(images)] = extract_ss(net, input)
                else:
                    vecs[:, i] =extract_ss(net, input)
            else:
                if batchsize!=1:
                    if (i+1) != len(loader):
                        vecs[:, i*batchsize:(i+1)*batchsize] = extract_ms(net, input, ms, msp, batchsize)
                    else:
                        vecs[:, i * batchsize:len(images)] = extract_ms(net, input, ms, msp, len(images)-i*batchsize, morebatch=True)
                else:
                    vecs[:, i] =extract_ms(net, input, ms, msp, batchsize)
...
def extract_ms(net, input, ms, msp, batchsize, morebatch=False):

    v = torch.zeros(net.meta['outputdim'], batchsize)
    v=v.squeeze()
    for s in ms: 
        if s == 1:
            input_t = input.clone()
    ...
    v = v.pow(1./msp)
    v /= v.norm(dim=0)
    if batchsize==1 and morebatch==True:
        v=v.unsqueeze(1)
    return v
