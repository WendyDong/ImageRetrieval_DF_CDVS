...
def compute_top_match(ranks ,gnd):
    top_match = 0
    nq = len(gnd)  # number of queries
    nempty = 0
    ...
    for i in np.arange(nq):
        ...
        # sorted positions of positive and junk images (0 based)
        pos = [np.in1d(ranks[0, i], qgnd)]
        if any(pos):
            top_match+=1

    top_match = float(top_match) / (nq - nempty)
    return top_match
def compute_map_and_print(dataset, ranks, gnd, kappas=[1, 5, 10]):
    
    # old evaluation protocol
    if dataset.startswith('oxford5k') or dataset.startswith('paris6k') or dataset=='kaggle_test3' or 'cdvs' in dataset or 'cub200' in dataset:
        map, aps, mprH, prsH = compute_map(ranks, gnd,kappas)
        print('>> {}: mAP {:.2f}'.format(dataset, np.around(map*100, decimals=2)))
        print('>> {}: mp@k{} {}'.format(dataset, kappas, np.around(mprH * 100, decimals=2)))
    # if 'cdvs' in dataset:
    #     top_match = compute_top_match(ranks,gnd)
    #     print('>> {}: Top Match {:.2f}'.format(dataset, np.around(top_match*100, decimals=2)))
